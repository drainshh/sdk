...
void features::visuals::player::animate(player_t* entity, const int index) {
	const float step = 255.f * (1.f / 2.5f) * interfaces::globals->frame_time;
...
void features::visuals::player::draw_box(player_t* entity, bbox_t bbox) {
	...
	if (c::visuals::playerbox) {
		...
		case 0:
			...
			color_t box_clr = color_t(c::visuals::playerbox_color[0], c::visuals::playerbox_color[1], c::visuals::playerbox_color[2]);
		if (menu::experimental_femboy_theme_active)
			box_clr = color_t(255, 182, 193);
		else if (menu::experimental_fight_club_mode)
			box_clr = color_t(150, 0, 0);

		im_render.drawrect(bbox.x, bbox.y, bbox.w, bbox.h, box_clr.get_color_edit_alpha_int(fade.at(entity->index())));
		...
		case 1:
			...
			color_t corner_clr = color_t(c::visuals::playerbox_color[0], c::visuals::playerbox_color[1], c::visuals::playerbox_color[2]);
			if (menu::experimental_femboy_theme_active)
				corner_clr = color_t(255, 182, 193);
			else if (menu::experimental_fight_club_mode)
				corner_clr = color_t(150, 0, 0);

			im_render.drawcornerbox(bbox.x, bbox.y, bbox.w, bbox.h, corner_clr.get_color_edit_alpha_int(fade.at(entity->index())));
			if (c::visuals::player_box_outline[1])
				im_render.drawcornerbox(bbox.x + 1, bbox.y + 1, bbox.w - 2, bbox.h - 2, color_t(0, 0, 0).get_color_edit_alpha_int(fade.at(entity->index())));
			break;
		}
...
void features::visuals::player::backtrack_dot(player_t* entity) {
	...
	for (int i = 1; i < backtrack.data[entity->index()].size(); i++) {//loop bt records
		...
		if (previous_screenpos.is_valid() && !previous_screenpos.is_zero()) {
			...
			im_render.drawrectfilled(screen_pos.x, screen_pos.y, 3, 3, color_t(c::visuals::backtrack_dot_clr[0], c::visuals::backtrack_dot_clr[1], c::visuals::backtrack_dot_clr[2]).get_color_edit_alpha_int(fade.at(entity->index())), 0.1f);
	...
	if (c::visuals::skeleton) {
		...
		for (int i = 0; i < p_studio_hdr->bones_count; i++) {
			...
			if (bone && (bone->flags & BONE_USED_BY_HITBOX) && (bone->parent != -1)) {
				...
				if (interfaces::debug_overlay->world_to_screen(v_parent, s_parent) && interfaces::debug_overlay->world_to_screen(v_child, s_child)) {
					color_t skeleton_clr = color_t(c::visuals::skeletonesp_clr[0], c::visuals::skeletonesp_clr[1], c::visuals::skeletonesp_clr[2]);
					if (menu::experimental_femboy_theme_active)
						skeleton_clr = color_t(255, 182, 193);
					else if (menu::experimental_fight_club_mode)
						skeleton_clr = color_t(150, 0, 0);

					im_render.drawline(s_parent[0], s_parent[1], s_child[0], s_child[1], skeleton_clr.get_color_edit_alpha_int(fade.at(entity->index())), 2.5f);
				}
			}
		}
	}
