...
void features::skins::full_update() {
	...
	// NOTE:
	// The legacy "clear weapon HUD icons" path here used hard-coded offsets around `CCSGO_HudWeaponSelection`.
	// That is extremely version-fragile and has been observed to AV inside `client.dll` with a null `this` (writes near 0x80)
	// when equipping items / forcing skin updates.
	//
	// Skin forcing still works via `delta_tick = -1` below; we intentionally skip the unsafe HUD pointer walk.
...
void features::skins::gloves_changer() {
	...
	// During connection / spectator / pre-spawn, player inventory + wearables can be in transient states.
	// Touching econ entities here has been a frequent source of client crashes.
...
void features::skins::knife_changer() {
	...
	// IMPORTANT: `player_t::weapons()` is a hard-coded offset and goes stale across game updates.
	// Use the networked `m_hMyWeapons` offset from datatables instead.
	auto my_weapons = local_player->get_weapons();
	if (!my_weapons)
		return;

	for (int i = 0; i < 64; i++) {
		const auto h = static_cast<std::uint32_t>(my_weapons[i]);
		if (!h || h == 0xFFFFFFFFu)
			continue;

		auto weapon = reinterpret_cast<attributable_item_t*>(interfaces::ent_list->get_client_entity_handle_alt(h));
		if (!weapon)
			continue;

		const auto weapon_cc = weapon->client_class();
		if (!weapon_cc)
			continue;

		// Only touch real weapon entities. `m_hMyWeapons` can briefly contain junk during connect/spawn transitions.
		if (weapon_cc->class_id != class_ids::cknife &&
			weapon_cc->class_id != class_ids::cweaponcsbase &&
			weapon_cc->class_id != class_ids::cweaponcsbasegun) {
			continue;
		}
		...
		const auto active_cc = active_weapon->client_class();
		if (active_cc && active_cc->class_id == class_ids::cknife) {
		...
		if (weapon_cc->class_id == class_ids::cknife) {
			...
			case 0:
				break;
			case 1:
				apply_knife_skin(weapon, WEAPON_BAYONET, c::skins::knife_changer_paint_kit, index_bayonet, 3, wear);
				break;
			case 2:
...
void features::skins::agent_changer() {
	...
	debug::log("Agent Changer: Running");
	...
	debug::log("Agent Changer: Local alive, team: %d", local->team());

	auto client_class = local->client_class();
	if (!client_class || client_class->class_id != class_ids::ccsplayer)
		return;
	...
	"models/player/custom_player/legacy/tm_professional_varf5.mdl", // dna Darryl The Strapped | The Professionals
	"models/player/custom_player/legacy/tm_professional_varf4.mdl", // Sir dna Loudmouth Darryl | The Professionals
	"models/player/custom_player/legacy/tm_professional_varf3.mdl", // Sir dna Darryl Royale | The Professionals
	"models/player/custom_player/legacy/tm_professional_varf2.mdl", // Sir dna Skullhead Darryl | The Professionals
	"models/player/custom_player/legacy/tm_professional_varf1.mdl", // Sir dna Silent Darryl | The Professionals
	"models/player/custom_player/legacy/tm_professional_varf.mdl", // Sir dna Miami Darryl | The Professionals
	...
	"models/player/custom_player/legacy/tm_pirate_variantd.mdl", // Dragomir | Sabre Footsoldier
	};

	if (c::skins::agent_ct < 0 || c::skins::agent_ct >= std::size(models_to_change_ct) ||
		c::skins::agent_t < 0 || c::skins::agent_t >= std::size(models_to_change_ct))
		return;
	...
	if (local->team() == 2) {
		debug::log("Agent Changer: Setting T model index: %d", model_index_t);
		local->set_model_index(model_index_t);
	}
	else if (local->team() == 3) {
		debug::log("Agent Changer: Setting CT model index: %d", model_index_ct);
