...
#include "interfaces/c_physicscollision.h"

namespace g {
	inline player_t* local = nullptr;
	inline c_usercmd* cmd = nullptr;
	inline const char* last_hook = "none";
	inline const char* last_event = "none";
	inline int last_frame_stage = -1;
}
...
public:
	...
	NETVAR("DT_Precipitation", "m_vecMins", mins, vec3_t)
	NETVAR("DT_Precipitation", "m_vecMaxs", maxs, vec3_t)
	NETVAR("DT_Precipitation", "m_fEffects", effects, uint32_t)
	NETVAR("DT_Precipitation", "m_nModelIndex", get_model_index, int)
	NETVAR("DT_BaseEntity", "m_nSolidType", solid_type, int)
	NETVAR("DT_BaseEntity", "m_usSolidFlags", solid_flags, unsigned short)
	NETVAR("DT_Precipitation", "m_clrRender", clr_render, int)
...
public:
	...
	bool is_reloading() {
		if (!this)
			return false;
		static auto pattern_addr = find_pattern("client.dll", "C6 87 ? ? ? ? ? 8B 06 8B CE FF 90");
		if (!pattern_addr)
			return false;
		static auto inReload = *(uint32_t*)(pattern_addr + 2);
		if (!this)
			return false;
...
public:
	...
	vec3_t& get_velocity() {
		static vec3_t null_vec = { 0,0,0 };
		if (!this) return null_vec;
		return *(vec3_t*)((DWORD)this + 0x114);
	}

	player_t* local_or_spectated() {
		if (!interfaces::engine->is_in_game()) return nullptr;
	...
	bool isvisiblebone(int bone) {
		auto local_player = reinterpret_cast<player_t*>(interfaces::ent_list->get_client_entity(interfaces::engine->get_local_player()));
		if (!local_player || !this) return false;
		ray_t ray;
		trace_t tr;
	...
	bool is_enemy() {
		...
		auto local = get_local_player(true);
		if (!local) return false;

		if (in_dangerzone()) {
			return this->survival_team() != local->survival_team() || local->survival_team() == -1;
		}
		else {
			return this->team() != local->team();
		}
	}
	...
	void set_abs_velocity(const vec3_t abs_velocity)
	{
		using fn = void(__thiscall*)(void*, const vec3_t& abs_velocity);
		static fn velocity = (fn)find_pattern("client.dll", "55 8B EC 83 E4 F8 83 EC 0C 53 56 57 8B 7D 08 8B F1 F3");

		velocity(this, abs_velocity);
	}
