...
extern "C" LONG WINAPI CrashHandler(EXCEPTION_POINTERS* exceptionInfo);
...
namespace sdk {
	namespace hooks {
		...
		namespace level_init_pre_entity {
			...
			inline fn ofunc;
			void __stdcall level_init_pre_entity(const char* map);
		}
		...
		namespace fire_event {
			using fn = bool(__thiscall*)(i_game_event_manager2*, i_game_event*, bool);
			inline fn ofunc;
			bool __stdcall fire_event(i_game_event* gameEvent, bool bDontBroadcast);
		}
