...
#include "functions/precip_register.h"
#include "functions/precache_precipitation.h"
#include "functions/precipitation_callback.h"
#include "functions/precip_particle_register.h"
...
// В начало hooks.cpp (после инклудов)
extern void update_media_tag();

// В то место, где ты хочешь видеть саму функцию (обычно перед хуками)
void update_media_tag()
{
	// Используем переменную из твоего неймспейса конфига
	if (!c::misc::music_clantag)
		return;

	// Объявляем функции, чтобы hooks.cpp их видел
	extern std::string get_current_media_info();
	extern std::string animate_tag(std::string text);
	extern void apply_clan_tag(const char* tag, const char* name);

	static long next_update = 0;
	if (GetTickCount() > next_update) {
		std::string track = get_current_media_info();
		std::string animated = animate_tag(track);
		apply_clan_tag(animated.c_str(), "DNA Music");
		next_update = GetTickCount() + 375;
	}
}
...
void sdk::hooks::init() {
	...
	// Регистрация оригинального хука CreateMove [cite: 2, 7]
	...
	if (MH_CreateHook(find_pattern(("client.dll"), ("55 8B EC 83 E4 F8 B8 ? ? ? ? E8 ? ? ? ? 53 8B D9 8B 0D")), &push_notice::push_notice, (void**)&push_notice::ofunc))
		printf(("push_notice hook failed.\n"));
	...
	// Weather / precipitation hooks. Keep these BEFORE the single MH_EnableHook(MH_ALL_HOOKS) below.
	auto particle_simulate = find_pattern(
		"client.dll",
		"55 8B EC 83 E4 F8 83 EC 30 56 57 8B F9 0F 28 E1 8B 0D ? ? ? ? F3 0F 11 64 24 ? 89 7C 24 18 8B 81"
	);

	auto precip_register = find_pattern(
		"client.dll",
		"55 8B EC 53 8B 5D ? 8B CB 56 8B 75 ? 57 8B 7D ?"
	);
	debug::log("PRECIP REGISTER SIG: %p", precip_register);

	auto precipitation_callback = find_pattern(
		"client.dll",
		"55 8B EC A1 ? ? ? ? 56 68 ? ? ? ? 8B 08 8B 01 FF 50 ? 85 C0"
	);
	debug::log("CPRECIPITATION CALLBACK SIG: %p", precipitation_callback);

	//if (precip_register) {
		//MH_CreateHook(
			//precip_register,
			//&sdk::hooks::precip_register::hook,
		//	reinterpret_cast<void**>(&sdk::hooks::precip_register::ofunc)
		//);
		//debug::log("HOOK OK: precip_register %p", precip_register);
	//}
	//else {
	//	debug::log("HOOK FAIL: precip_register pattern not found");
	//}

	auto precip_particle_register = find_pattern(
		"client.dll",
		"68 ? ? ? ? 6A 01 FF 10 8B 03 8B CB 6A 00 57 56 68 ? ? ? ?"
	);
	debug::log("PRECIP PARTICLE REGISTER SIG: %p", precip_particle_register);

	//if (precip_particle_register) {
		//MH_CreateHook(
			//precip_particle_register,
			//&sdk::hooks::precip_particle_register::hook,
			//reinterpret_cast<void**>(&sdk::hooks::precip_particle_register::ofunc)
		//);
		//debug::log("HOOK OK: precip_particle_register %p", precip_particle_register);
	//}
	//else {
		//debug::log("HOOK FAIL: precip_particle_register pattern not found");
	//}

	auto precache_precipitation = find_pattern(
		"client.dll",
		"FF 50 ? 85 C0 75 ? 33 FF EB ? 8D 78 ? 83 E7 ? 89 47 ? 68 ? ? ? ? 6A 00 57 E8 ? ? ? ?"
	);
	...
	if (particle_simulate) {
		MH_CreateHook(
			particle_simulate,
			&particle_collection_simulate::hook,
			reinterpret_cast<void**>(&particle_collection_simulate::ofunc)
		);
		debug::log("HOOK OK: CParticleCollection::Simulate %p", particle_simulate);
	}
	else {
	...
	if (MH_EnableHook(MH_ALL_HOOKS) != MH_OK)
		printf(("[-] dna failed to initialize hooks\n"));
	else
		printf(("[+] dna initialized hooks\n"));

	events.setup();

	window = FindWindowW((L"Valve001"), NULL);
	if (!window)
		printf(("[-] dna failed to get game window\n"));
	else
		printf(("[+] dna found game window\n"));
...
void sdk::hooks::unload() {
	...
	MH_Uninitialize();
	SetWindowLongPtr(window, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(wndproc_original));
}
